...
function getWords() {
  return axios
    .get(
      `http://api.wordnik.com:80/v4/words.json/randomWords?hasDictionaryDef=true&minCorpusCount=0&maxCorpusCount=-1&minDictionaryCount=1&maxDictionaryCount=-1&minLength=5&maxLength=-1&limit=${data.length}&api_key=a2a73e7b926c924fad7001ca3111acd55af2ffabf50eb4ae5`
    )
...
function matchDefinitions() {
  getWords()
    .then(resp =>
      resp.map((word, index) => {
        getDefinitions(word.word).then(resp => {
          let defs = resp;
          defs.map(def => {
